...
package com.ihomey.linkuphome.update;
...
class UpdateAgent implements ICheckAgent, IUpdateAgent, IDownloadAgent {
    ...
    private class DefaultUpdatePrompter implements IUpdatePrompter {
        ...
        public void prompt(IUpdateAgent agent) {
            ...
            String content = String.format(mContext.getString(R.string.version_latest) + "：%1$s\n" + mContext.getString(R.string.app_size) + "：%2$s\n\n" + mContext.getString(R.string.app_update_content) + "\n%3$s", info.versionName, size, info.updateContent);
            ...
            if (info.isForce) {
                tv.setText(mContext.getString(R.string.app_update_need) + "\n\n" + content);
                dialog.setButton(DialogInterface.BUTTON_POSITIVE, mContext.getString(R.string.ok), listener);
            } else {
                tv.setText(content);
                dialog.setButton(DialogInterface.BUTTON_POSITIVE, mContext.getString(R.string.app_update_now), listener);
                dialog.setButton(DialogInterface.BUTTON_NEGATIVE,  mContext.getString(R.string.app_update_ignore_now), listener);
                if (info.isIgnorable) {
                    dialog.setButton(DialogInterface.BUTTON_NEUTRAL, mContext.getString(R.string.app_update_ignore), listener);
                }
            }
            dialog.show();
        }
    ...
    private static class DefaultFailureListener implements OnFailureListener {
        ...
        public void onFailure(UpdateError error) {
            ...
            if (error.code > 3000 && error.code < 3011) {
                Toast.makeText(mContext, R.string.app_update_download_fail, Toast.LENGTH_LONG).show();
            }
        }
    }
    ...
    private static class DefaultDialogDownloadListener implements OnDownloadListener {
        ...
        public void onStart() {
            if (mContext instanceof Activity && !((Activity) mContext).isFinishing()) {
                ...
                dialog.setMessage(mContext.getString(R.string.app_update_downloading));
