...
class C3Controller(val device: Device): CommonController, ColorController {
    ...
    override fun setBrightness(brightness: Int) {
        val commandPrefix="C101F1C003C2" + (if (brightness >= 1) Integer.toHexString(brightness+15) else "0" + Integer.toHexString(brightness+15))+ "00"
    ...
    override fun setOnOff(isOn: Boolean) {
        if(CSRMeshServiceManager.getInstance().isConnected()) DataModelApi.sendData(device.instructId, decodeHex((if (isOn) "C101F1C003C164002816" else "C101F1C003C10000C416").toCharArray()), false)
    }
