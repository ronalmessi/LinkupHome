...
class SigMeshV1RepeatTimerController(val device: Device): RepeatTimerController {
    ...
    override fun syncTime() {
        ...
        if(PlSigMeshService.getInstance().isMeshReady) {
            PlSigMeshService.getInstance().vendorUartSend(device.pid.toShort(), Util.hexStringToBytes(dateCommand), Util.PL_DEFAULT_APP_KEY_INDEX)
        }
        ...
        if(PlSigMeshService.getInstance().isMeshReady) {
            Handler().postDelayed({ PlSigMeshService.getInstance().vendorUartSend(device.pid.toShort(), Util.hexStringToBytes(timeCommand), Util.PL_DEFAULT_APP_KEY_INDEX)},1000)
        }
    }
