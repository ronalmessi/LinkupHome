...
class DeviceListAdapter(layoutId: Int) : BaseQuickAdapter<SingleDevice, BaseViewHolder>(layoutId) {
    override fun convert(helper: BaseViewHolder, item: SingleDevice) {
        ...
        if(item.device.type==8){
            ...
            helper.setImageBitmap(R.id.lamp_device_mesh_iv_icon, srcBitmap)
            helper.getView<ImageView>(R.id.lamp_device_mesh_iv_icon).scaleX=1f
            helper.getView<ImageView>(R.id.lamp_device_mesh_iv_icon).scaleY=1f
            helper.getView<ImageView>(R.id.lamp_device_mesh_iv_icon).setPadding(mContext.dip2px(8f),mContext.dip2px(12f),mContext.dip2px(1f),mContext.dip2px(8f))
        }else{
