...
abstract class DeviceDao {
    ...
    @Transaction
    @Query("SELECT * FROM device2 WHERE zoneId = :zoneId and roomId =0 and type!=0 order by type asc,id asc")
    abstract fun getPagingUnBondedDevices(zoneId: Int): DataSource.Factory<Int, Device>

    @Transaction
    @Query("SELECT * FROM device2 WHERE zoneId = :zoneId and roomId = :roomId and type!=0 order by type asc,id asc")
    ...
    @Query("DELETE FROM device2 WHERE type!=0 and zoneId = :zoneId")
