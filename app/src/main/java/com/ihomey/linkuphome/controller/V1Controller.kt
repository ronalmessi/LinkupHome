...
class V1Controller : Controller() {
    ...
    override fun setRepeatTimer(deviceId: Int, minuteValue: Int, hourValue: Int, isOpenTimer: Boolean, isOn: Boolean, isRepeat: Boolean) {
        val code_lawn_timer_prefix = CODE_LIGHT_TIMER_BASE + (if (isOpenTimer) "C5" else "C6") + (if (isOn) {
            if (isRepeat) "80" else "00"
        } else "FF") + if (!isOn) "0000" else {
            (if (hourValue >= 10) "" + hourValue else "0$hourValue") + (if (minuteValue >= 10) "" + minuteValue else "0$minuteValue")
        }
