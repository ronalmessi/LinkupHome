...
public class GuideBuilder {
   ...
   * 设置目标view
   */
  public GuideBuilder setFullView(View v) {
    if (mBuilt) {
      throw new BuildException("Already created. rebuild a new one.");
    } else if (v == null) {
      throw new BuildException("Illegal view.");
    }
    mConfiguration.mFullView = v;
    return this;
  }
   ...
   * @return GuideBuilder
   */
  public GuideBuilder setHighTargetMarginTop(int marginTop) {
    if (mBuilt) {
      throw new BuildException("Already created. rebuild a new one.");
    } else if (marginTop < 0) {
      mConfiguration.mMarginTop = 0;
    }
    mConfiguration.mMarginTop = marginTop;
    return this;
  }
